...
import { childAdolescentHero } from "@/data/child-adolescent";

export function ChildAdolescentHero() {
  return (
    <section
      aria-labelledby="child-adolescent-hero-heading"
      ...
      <Container className="relative grid items-center gap-10 py-14 md:py-16 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)] lg:gap-12 lg:py-20">
        <div className="home-reveal max-w-2xl">
          <p className="text-text-muted text-sm font-medium tracking-[0.18em] uppercase">
            {childAdolescentHero.eyebrow}
          </p>
          <h1
            id="child-adolescent-hero-heading"
            className="mt-4 text-[clamp(2rem,4.5vw,3.1rem)] leading-[1.15]"
          >
            {childAdolescentHero.heading}
          </h1>
          <p className="text-text mt-5 max-w-xl text-base leading-relaxed md:text-lg">
            {childAdolescentHero.supportingMessage}
          </p>
          <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
            <ButtonLink href={childAdolescentHero.primaryCta.href}>
              {childAdolescentHero.primaryCta.label}
            </ButtonLink>
            <ButtonLink
              href={childAdolescentHero.secondaryCta.href}
              variant="secondary"
            >
              {childAdolescentHero.secondaryCta.label}
            </ButtonLink>
          </div>
        </div>
        ...
        <div className="home-reveal home-reveal-delay min-h-[15rem] lg:min-h-[24rem]">
          <AboutNatureVisual
            caption="Gentle guidance. Growing minds."
